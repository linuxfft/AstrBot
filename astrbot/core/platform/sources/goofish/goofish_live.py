...
class GoofishClient(object):
    def __init__(self, platform_config: dict, platform_settings: dict) -> None:
        ...
        self.ws: ClientConnection | None = None  # websocket

        # Token刷新相关配置
        self.token_refresh_interval = 3600  # Token刷新间隔，默认1小时
        self.token_retry_interval = 300  # Token重试间隔，默认5分钟
        ...
        # 消息过期时间配置
        self.message_expire_time = 300000

        # 人工接管关键词，从环境变量读取
        self.toggle_keywords = os.getenv("TOGGLE_KEYWORDS", "人工接管")
    ...
    async def handle_message(self, message_data: Dict, websocket: ClientConnection):
        ...
        try:
            ...
            await self._do_send_ack_msg(message_data, websocket)
            ...
            try:
                msg_type = message['3']['redReminder']
    ...
    async def handle_heartbeat_response(self, message_data: Dict):
        """处理心跳响应"""
        try:
            if (
    ...
    async def send_msg(self, ws: ClientConnection, cid, toid, text: str):
        text = {
            "contentType": 1,
            "text": {
                "text": text
        ...
        await ws.send(message=json.dumps(msg))

    async def init(self, ws: ClientConnection):
